...
class UsageLimiter:
    ...
    def fail(
        ...
        *,
        ...
        return self.repository.fail(reservation, usage, policy=policy or self.policy)
    ...
    def _record_failure(self, reservation: UsageEvent, exc: BaseException) -> None:
        ...
        try:
            if usage is not None:
                ...
                # Price it at this model's rates: recording a luna failure at
                # the global default understates the global cap by about 3x.
                self.limiter.fail(reservation, usage, policy=self.policy)
                ...
                # the hole.
                self.limiter.fail(reservation, None, policy=self.policy)
        except Exception:
            logger.exception(
